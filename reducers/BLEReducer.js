  ...
  switch (action.type) {
    case 'ADD_BLE':
      if (
        ...
        return update(state, {
          BLEList: {
            $set: [...state.BLEList, action.device],
          },
        });
      }
    ...
    case 'BIOS_SET_READ_BUFFER':
      return update(state, {biosReadBuffer: {$set: action.readBuffer}});

    case 'BIOS_SET_READ_LAST_JSON':
      return update(state, {biosReadLastJson: {$set: action.readJson}});
